...
let bound_region_unification _ = 
  ...
  let pre_msg = Printf.sprintf "Regions before unification must not be equal! (%s, %s)" sr1 sr2 in
  let _ = assert_bool pre_msg (Region.equal r1 r2 |> not) in

  let post_msg = Printf.sprintf "Regions after unification must be equal! (%s, %s)" sr1 sr2 in
  ...
  let uniq_msg = Printf.sprintf "Region.uniq_of ignores unification (%s, %s)" sid1 sid2  in
